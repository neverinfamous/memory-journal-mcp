...
const SEED_FINGERPRINTS = [
    ...
    { label: 'S1', fingerprint: 'Redesigned the authentication architecture for the OAuth 2.1 module', db: 'personal' },
    { label: 'S2', fingerprint: 'Improved error handling in the database adapter layer with typed error classes', db: 'personal' },
    { label: 'S3', fingerprint: 'Deploy new release candidate to the CDN edge network', db: 'personal' },
    { label: 'S4', fingerprint: 'Released v5.0 with breaking API changes and migration guide', db: 'personal' },
    { label: 'S5', fingerprint: 'Deploy to staging environment failed — rollback initiated', db: 'personal' },
    { label: 'S6', fingerprint: "The test's scope was expanded to cover 100% of edge cases", db: 'personal' },
    // 0.2 Filter & GitHub-Linked Entries
    { label: 'S7', fingerprint: 'Investigated performance regression in issue #44 — root cause was N+1 queries', db: 'personal' },
    { label: 'S8', fingerprint: 'Code review feedback from PR #67 merged — refactored authentication middleware', db: 'personal' },
    { label: 'S9', fingerprint: 'CI workflow run completed — all 910 tests passing across 3 test suites', db: 'personal' },
    { label: 'S10', fingerprint: 'Personal reflection on improving development velocity and reducing technical debt', db: 'personal' },
    // 0.3 Team & Cross-DB Entries
    { label: 'S11', fingerprint: 'Architecture decision: adopted event-driven design for webhook processing', db: 'both' },
    { label: 'S12', fingerprint: 'Team standup: discussed authorization flow improvements and deploy pipeline', db: 'team' },
    // 0.4 Cross-Project Insights Seed
    { label: 'S13', fingerprint: 'Completed sprint planning for project #5: scoped auth and deploy milestones', db: 'personal' },
    { label: 'S14', fingerprint: 'Project #5 retrospective: identified bottlenecks in deployment pipeline and action items', db: 'personal' },
    { label: 'S15', fingerprint: 'Team kickoff for project #5: aligned on goals and delivery timeline', db: 'team' },
    { label: 'S16', fingerprint: 'Project #5 mid-sprint check-in: auth module ahead of schedule, deploy pipeline at risk', db: 'team' },
    { label: 'S17', fingerprint: 'Project #5 release review: all acceptance criteria met, feature flags enabled for rollout', db: 'team' },
]
...
function findSeedEntries(db, fingerprints) {
    ...
    for (const { label, fingerprint } of fingerprints) {
        const rows = db.prepare('SELECT id FROM memory_journal WHERE deleted_at IS NULL AND content LIKE ?').all(`%${fingerprint}%`)
...
function processDatabase({ db, label, seedFingerprints }) {
    ...
    const allActiveRows = db.prepare(`
    ...
    for (const row of allActiveRows) {
        let isProtectedProject = row.project_number !== null && PROTECTED_PROJECT_NUMBERS.includes(row.project_number)
        ...
        if (!isProtectedProject && row.content) {
            if (row.content.includes('# Session Summary') || row.content.includes('memory-journal-mcp') || row.content.includes('postgres-mcp')) {
                isProtectedProject = true
            }
        }
    ...
    if (toDelete.length > 0) {
        ...
        for (const row of toDelete.slice(0, 10)) {
            ...
            console.log(`    id=${row.id}  type=${row.entry_type}  project=${row.project_number ?? 'null'}\n      "${pv}${pv.length >= 80 ? '…' : ''}"`)
        }
...
async function main() {
    ...
    console.log('═'.repeat(60))
    console.log(`  Mode            : ${DRY_RUN ? '🔍 DRY RUN (pass --execute to apply)' : '🗑️  EXECUTE'}`)
    ...
    if (!personalMissing) {
        const result = processDatabase({
            ...
            seedFingerprints: SEED_FINGERPRINTS.filter(f => f.db === 'personal' || f.db === 'both')
        })
        personalDb.close()
