...
function validateAnnotations(tools) {
    ...
    for (const tool of tools) {
        ...
        if (a.destructiveHint === true && !DESTRUCTIVE_ALLOWLIST.has(tool.name)) {
            stats.destructiveMismatch.push(`${tool.name} (true but not in allowlist)`)
        }
        if (a.destructiveHint !== true && DESTRUCTIVE_ALLOWLIST.has(tool.name)) {
    ...
    if (stats.missingOpenWorld.length === 0) {
        ...
        fail('openWorldHint coverage', `missing on: ${stats.missingOpenWorld.join(', ')}`)
    }
    ...
    if (unexpectedOpenWorld.length === 0 && missingOpenWorld.length === 0) {
        ...
        if (missingOpenWorld.length > 0) {
            parts.push(`expected true but false/missing: ${missingOpenWorld.join(', ')}`)
        }
    ...
    if (stats.missingReadOnly.length === 0) {
        ...
        fail('readOnlyHint coverage', `missing on: ${stats.missingReadOnly.join(', ')}`)
    }
    ...
    if (stats.readOnlyMismatch.length === 0) {
        ...
        fail('readOnlyHint allowlist', `mismatches: ${stats.readOnlyMismatch.join('; ')}`)
    }

    // ── 6. All tools have explicit destructiveHint ──
    if (stats.missingDestructive.length === 0) {
        pass('destructiveHint coverage', `${stats.hasDestructiveHint}/${tools.length}`)
    } else {
        fail('destructiveHint coverage', `missing on: ${stats.missingDestructive.join(', ')}`)
    }

    // ── 6b. destructiveHint matches allowlist ──
    if (stats.destructiveMismatch.length === 0) {
        pass('destructiveHint allowlist', `${stats.destructiveTrue.length} destructive tools match`)
    } else {
        fail('destructiveHint allowlist', `mismatches: ${stats.destructiveMismatch.join('; ')}`)
    }
    ...
    if (stats.readOnlyNotIdempotent.length > 0) {
        warn('Read-only tools missing idempotentHint=true', stats.readOnlyNotIdempotent.join(', '))
    }
    ...
    if (stats.missingIdempotent.length === 0) {
        pass('idempotentHint coverage', `${stats.hasIdempotentHint}/${tools.length}`)
    } else {
        fail('idempotentHint coverage', `missing on: ${stats.missingIdempotent.join(', ')}`)
    }
    ...
    console.log(`│   openWorldHint:   ${pct(stats.hasOpenWorldHint, tools.length)}`)
    console.log(`│   readOnlyHint:    ${pct(stats.hasReadOnlyHint, tools.length)}`)
    console.log(`│   destructiveHint: ${pct(stats.hasDestructiveHint, tools.length)}`)
    console.log(`│   idempotentHint:  ${pct(stats.hasIdempotentHint, tools.length)}`)
    ...
    console.log(`│ BREAKDOWN: idempotentHint=true: ${stats.idempotentTrue.length}`)
    console.log('├──────────────────────────────────────────────────────────┤')
