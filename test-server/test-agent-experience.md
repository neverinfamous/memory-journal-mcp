...
Using `mj_execute_code`, create an entry and immediately retrieve it in a single call. Does the agent discover Code Mode from the instructions and use it unprompted?

> **What we're testing:** Can the agent discover `mj_execute_code` and the `mj.*` namespace from the instruction sections (which are present since `codemode` is enabled)?
...
> **What we're testing:** Can the agent find `get_vector_index_stats` (part of the `search` group) and interpret the results?
...
> **Note:** The agent needs to discover `get_kanban_board` and figure out project numbers. This tests whether the instructions + `memory://help/github` are sufficient.
