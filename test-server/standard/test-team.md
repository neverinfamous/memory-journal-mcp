...
| Test                | Command/Action                                                                                                       | Expected Result                                                   |
| ------------------- | -------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------- |
| Blocker flag        | `team_pass_flag(flag_type: "blocker", message: "FK constraint blocks migration", target_user: "@sarah")`             | `success: true`, `flag_type: "blocker"`, `target_user: "sarah"`   |
| FYI flag            | `team_pass_flag(flag_type: "fyi", message: "New lint rule added")`                                                   | `success: true`, no `target_user`                                 |
| Needs review        | `team_pass_flag(flag_type: "needs_review", message: "Auth refactor ready", target_user: "chris", issue_number: <N>)` | Entry has `issueNumber` set                                       |
| Help requested      | `team_pass_flag(flag_type: "help_requested", message: "Race condition on Windows")`                                  | `success: true`, all 4 vocabulary types accepted                  |
| With link           | `team_pass_flag(flag_type: "blocker", message: "Migration file", link: "src/db/migrations/005.ts")`                  | `auto_context.link` populated                                     |
...
| Invalid vocab       | `team_pass_flag(flag_type: "urgent", message: "test")`                                                               | `{ success: false, code: "VALIDATION_ERROR" }`, lists valid types |
| Missing message     | `team_pass_flag(flag_type: "blocker")`                                                                               | Structured error: message required                                |
| Missing flag_type   | `team_pass_flag(message: "test")`                                                                                    | Structured error: flag_type required                              |
| Empty params        | `team_pass_flag({})`                                                                                                 | Structured error: both required                                   |

### 14.17 Hush Protocol — Flag Resolution

| Test                  | Command/Action                                                        | Expected Result                                                 |
| --------------------- | --------------------------------------------------------------------- | --------------------------------------------------------------- |
| Resolve with comment  | `team_resolve_flag(flag_id: <id>, resolution: "Fixed by migration")`  | `success: true`, `resolved: true`, `resolution` set             |
...
| Idempotent re-resolve | `team_resolve_flag(flag_id: <id>, resolution: "Should not override")` | `success: true`, original resolution retained                   |
| Bare resolve          | `team_resolve_flag(flag_id: <new_flag>)` without resolution           | `success: true`, content contains `[RESOLVED]` (no comment)     |
| Nonexistent flag ID   | `team_resolve_flag(flag_id: 999999)`                                  | `{ success: false, code: "RESOURCE_NOT_FOUND" }`                |
| Non-flag entry        | `team_resolve_flag(flag_id: <non_flag_entry_id>)`                     | `{ success: false, code: "VALIDATION_ERROR" }`                  |
| Empty params          | `team_resolve_flag({})`                                               | Structured error: flag_id required                              |
...
- `team_pass_flag` strips `@` prefix from `target_user` before storage
- `team_pass_flag` generates `flag:{type}` and `@{user}` tags automatically
- `team_pass_flag` validates against configured vocabulary (rejects invalid types)
- `team_pass_flag` requires both `flag_type` and `message`
- `team_resolve_flag` transitions flag to resolved state with `[RESOLVED]` content marker
- `team_resolve_flag` is idempotent — re-resolving returns success with original state
- `team_resolve_flag` rejects non-flag entries with `VALIDATION_ERROR`
- `team_resolve_flag` returns `RESOURCE_NOT_FOUND` for nonexistent IDs
