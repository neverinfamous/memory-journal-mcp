...
describe('PullRequestsManager - coverage', () => {
    ...
    it('getPullRequest handles string error and draft', async () => {
        const mockClient = { 
            octokit: { pulls: { get: vi.fn().mockResolvedValue({ data: { 
                number: 1, title: 't', html_url: 'u', state: 'open',
                head: { ref: 'h' }, base: { ref: 'b' }, user: { login: 'u' },
                created_at: 'c', updated_at: 'u', additions: 1, deletions: 1, changed_files: 1
    ...
    it('getReviews handles string error and missing submitted_at', async () => {
        const mockClient = { 
            octokit: { rest: { pulls: { listReviews: vi.fn().mockResolvedValue({ data: [
                { id: 1, user: { login: 'user' }, state: 'APPROVED', body: 'body' }
            ] }) } } }, 
            getCached: vi.fn().mockReturnValue(undefined), 
            setCache: vi.fn() 
        } as unknown as GitHubClient
        ...
        // error handling
        const errClient = { 
            octokit: { rest: { pulls: { listReviews: vi.fn().mockRejectedValue('String Error') } } }, 
            getCached: vi.fn().mockReturnValue(undefined), 
            setCache: vi.fn() 
        } as unknown as GitHubClient
        const errManager = new PullRequestsManager(errClient)
    ...
    it('getReviewComments handles string error', async () => {
        const errClient = { 
            octokit: { rest: { pulls: { listReviewComments: vi.fn().mockRejectedValue('String Error') } } }, 
            getCached: vi.fn().mockReturnValue(undefined), 
            setCache: vi.fn() 
        } as unknown as GitHubClient
        const errManager = new PullRequestsManager(errClient)
