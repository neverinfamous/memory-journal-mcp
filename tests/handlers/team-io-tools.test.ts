...
describe('getTeamIoTools', () => {
    ...
    describe('team_export_markdown', () => {
        it('should return error when teamDb not configured', async () => {
            ...
            const handler = getTeamIoTools(context).find((t) => t.name === 'team_export_markdown')!.handler
        ...
        it('should fetch by dates when start_date is provided', async () => {
            ...
            const result = await handler({ output_dir: '/tmp/team-out', start_date: '2025-01-01' }) as any
            expect((context.teamDb as any).searchByDateRange).toHaveBeenCalled()
        })

        it('should handle errors during export', async () => {
            const context = createMockContext()
            context.teamDb.getRecentEntries = vi.fn().mockImplementation(() => { throw new Error('Export error') })
            const handler = getTeamIoTools(context as never).find((t) => t.name === 'team_export_markdown')!.handler
    ...
    describe('team_import_markdown', () => {
        it('should return error when teamDb not configured', async () => {
            const context = { progress: null } as any
            const handler = getTeamIoTools(context).find((t) => t.name === 'team_import_markdown')!.handler
