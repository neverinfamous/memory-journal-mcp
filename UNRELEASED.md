...
- **Direct MCP**: Validated Phase 5 Admin & Backup tools natively via direct MCP calls, confirming structured error returns on tag merges, safe backup orchestration, path-traversal safety, and accurate data restoration without any required logic remediation.
- **Direct MCP**: Validated Phase 2 Entry CRUD operations natively via direct MCP calls, confirming creation with metadata linkage, structured error paths, comprehensive reads (with relationships/importance breakdown), successful updates, and clean deletion workflows without any required logic remediation.
- **Direct MCP**: Validated Phase 1 Core Infrastructure natively via direct MCP calls and integration scripts, confirming server health, briefing json structures, instruction level token ordering, Github openWorldHint tool annotations (22), and GitHub status without any required logic remediation.
- **Direct MCP**: Validated Phase 4 Relationships & Visualization tools natively via direct MCP calls, confirming creation of 5 connection types (including causal variants), error path handling for invalid mapping IDs, duplication constraints, and successful rendering of `visualize_relationships` alongside `memory://graph/recent` outputs without any required logic remediation.
- **Direct MCP**: Validated IO & Markdown Interoperability tools natively via direct MCP calls, confirming JSON/Markdown export mapping, dry-run directory parsing, correct Frontmatter compilation, and structured active path traversal defenses without any required logic remediation.
- **Direct MCP**: Validated Phase 6 Automated Scheduler internally via HTTP transport integration script, confirming successful initialization and execution of all 4 periodic background jobs (backup, vacuum, rebuild-index, digest) with accurate run counts and transport-specific boundaries (disabled in stdio) without any required logic remediation.
- **Direct MCP**: Validated Phase 3 Search, Semantics, and Analytics tools natively via direct MCP calls, confirming FTS5 search patterns, hybrid RRF auto-mode, phrase queries, graceful fallbacks for SQLite special characters, metadata integrity mapping, date range endpoints, cross-DB source merging, and importance-based priority sorting parameters without any required logic remediation.
- **Direct MCP**: Validated Phase 3 Semantic Search & Analytics natively via direct MCP calls, confirming exact `entry_id` relative embeddings, structured quality/advisory hint separation, rigid similarity thresholds, valid cross-project insight structures (`min_entries` filters and multi-dimensions), exact chronological boundaries (`start_date`/`end_date`), and precise dynamic vector index additions without any required logic remediation.
