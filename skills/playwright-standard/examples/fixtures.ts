...
export const test = base.extend<MyFixtures>({
  ...
  authenticatedUser: async ({ browser }: { browser: Browser }, use: (r: User) => Promise<void>) => {
...
class TodoPage {
  ...
  async removeAll() { /* ... */ }
  async addTodo(text: string) { await this.page.fill('input', text); await this.page.click('text=Add'); }
}
