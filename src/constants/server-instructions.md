...
- **`semantic_search` thresholds**: Default similarity threshold is 0.25. For broader matches, try 0.15-0.2. Higher values (0.4+) return only very close semantic matches. A quality floor of 0.5 controls `hint_on_empty`: if all results score below 0.5, a hint is included indicating results may be noise.
